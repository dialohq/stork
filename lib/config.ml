let home =
  ...
  with Not_found ->
    (try Ok (Unix.getpwuid (Unix.getuid ())).Unix.pw_dir
     with Unix.Unix_error _ | Not_found ->
       if Sys.win32 then
         try Ok (Sys.getenv "AppData")
         with Not_found -> Error (Stork_error.missing_env "AppData")
       else Error (Stork_error.missing_env "HOME"))
