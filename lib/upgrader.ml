...
let classify_item
  ...
  | _, Some old_item when old_item = new_item ->
    let () = print_endline (ModuleItem.show old_item) in
    let () = print_endline (ModuleItem.show new_item) in
...
let classified_type_to_strings
  ...
  | Same name ->
    let impl =
      [%string
        "let make_$name: $old_version.$name -> $new_version.$name = Obj.magic"]
    in
    let intf =
      [%string "val make_$name: $old_version.$name -> $new_version.$name"]
    in
    impl :: impl_list, intf :: intf_list, user_intf_list
  | New name ->
    let intf =
      [%string
        "val make_$name: $old_version.$old_doc_type -> $new_version.$name"]
    in
  ...
  | Modified name ->
    let intf =
      [%string
        "val\n\
        \   make_$name: $old_version.$name -> $old_version.$old_doc_type -> \
         $new_version.$name"]
    in
    impl_list, intf :: intf_list, intf :: user_intf_list
  | TransitivelyModified { record_name; unmodified_fields; transitive_fields }
    ...
    let intf =
      [%string
        "val make_$record_name: $old_version.$record_name -> \
         $old_version.$old_doc_type -> $new_version.$record_name"]
    in
    let unmodified_fields =
      List.fold_left
        ~f:(fun acc field ->
          ...
          | true ->
            acc ^ [%string "$field = make_$field old_record.$field; "]
          | false ->
    ...
    let transitive_fields =
      List.fold_left
        ~f:(fun acc field ->
          acc ^ [%string "$field = make_$field old_record.$field old_doc; "])
        ~init:""
        transitive_fields
    in
    let impl =
      [%string
        "let make_$record_name (old_record: $old_version.$record_name)  \
...
let make ~prefix ~old_file ~old_file_version ~new_file ~new_file_version =
  ...
  let _additional_fns =
    [%string
      "$(String.capitalize_ascii prefix)$(string_of_int \
       old_file_version)_to_$(string_of_int new_file_version)_additional"]
  in
  ...
  in
  let impl_list, intf_list, user_intf_list =
    List.rev impl_list, List.rev intf_list, List.rev user_intf_list
  in
  let old_version_t =
    [%string
      "$(String.capitalize_ascii prefix)$(string_of_int old_file_version)_t"]
  in
  let new_version_t =
    [%string
      "$(String.capitalize_ascii prefix)$(string_of_int new_file_version)_t"]
  in
  let user_fns_module =
    [%string
      "$(String.capitalize_ascii prefix)$(string_of_int \
       old_file_version)_to_$(string_of_int new_file_version)_$(user_fns)"]
  in
  let impl_header =
    [%string
...
module $new_version := $new_version_t|}]
  in
  let intf_list = intf_header :: intf_list in
  let impl_list = impl_header :: impl_list in
  let user_intf_list = intf_header :: user_intf_list in
  let folder = "lib/generated" in
  let path =
    [%string
      "$folder/$(prefix)$(string_of_int old_file_version)_to_$(string_of_int \
       new_file_version)_upgrader"]
  in
  ...
  File.write_string_list
    ~path:[%string "$folder/$(String.lowercase_ascii user_fns_module).mli"]
    user_intf_list
