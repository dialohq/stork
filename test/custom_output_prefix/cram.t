  ...
  > cat custom_output_prefix_1_t.ml
  > echo ---
  > cat custom_output_prefix_1_t.mli
  > echo ---
  > cat custom_output_prefix_1_j.ml
  > echo ---
  > cat custom_output_prefix_1_j.mli
  > echo ---
  > cat custom_output_prefix_bar_t.ml
  > echo ---
  > cat custom_output_prefix_bar_t.mli
  > echo ---
  > cat custom_output_prefix_bar_j.ml
  > echo ---
  > cat custom_output_prefix_bar_j.mli
  > echo ---
  > cat custom_output_prefix_upgrader_t.ml
  > echo ---
  > cat custom_output_prefix_upgrader_t.mli
  > echo ---
  > cat custom_output_prefix.ml
  > echo ---
  > cat custom_output_prefix.mli
  ...
  let converter = Custom_output_prefix_upgrader_t.From_1_to_bar.{ convert_employee;
  convert_employment;
  convert_employer;
  convert_company;
  convert_skill }
  ...
  let string_of_employee ?len x = Json.string_of_employee ?len { x with Types.version = "bar" }
  
  let employee_of_string s = match (get_version s) with
    | `String "bar" -> Json.employee_of_string s
    | `Int 1 -> convert_from_1_to_latest (Custom_output_prefix_1_j.employee_of_string s)
  ...
  let read_employee_from_file fname = match (get_version_from_file fname) with
    ...
    | `Int 1 -> convert_from_1_to_latest (Atdgen_runtime.Util.Json.from_file Custom_output_prefix_1_j.read_employee fname)
    | v -> invalid_arg ("Unknown document version: '" ^ version_to_string v ^ "'")
  
  let write_employee buf x = Json.write_employee buf { x with Types.version = "bar" }
  ---
